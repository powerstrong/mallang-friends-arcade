# 말랑프렌즈 퀴즈배틀

초등 상식 퀴즈 10문제를 풀며 가장 많이 맞힌 친구가 우승하는 온라인 실시간 게임입니다.

<!-- TODO: 스크린샷을 docs/media/mallang-quiz-battle.png 로 추가한 뒤 아래 줄 주석을 해제하세요.
![말랑프렌즈 퀴즈배틀 스크린샷](../media/mallang-quiz-battle.png) -->

---

## 기본 정보

| 항목 | 내용 |
|------|------|
| 인원 | 2~6명 |
| 플레이 방식 | 온라인 실시간 |
| 게임 시간 | 약 2분 |
| 상태 | PLAYABLE |
| 제작자 | powerstrong |

---

## 플레이 방법

1. 광장에서 퀴즈배틀 부스에 들어가거나, 방 코드로 직접 입장합니다.
2. 2명 이상이 모이면 게임이 시작됩니다.
3. 초등 상식 퀴즈 10문제가 차례로 출제됩니다.
4. 보기 4개 중 하나를 탭해서 정답을 선택합니다.
5. 빨리 맞힐수록 점수 보너스가 있습니다.
6. 10문제 후 점수 순위가 공개됩니다.

---

## 조작

- **모바일/PC**: 정답 보기 탭 또는 클릭

---

## 플레이하기

[https://web-game-lab.powerstrong.workers.dev/games/mallang-quiz-battle/index.html](https://web-game-lab.powerstrong.workers.dev/games/mallang-quiz-battle/index.html)
