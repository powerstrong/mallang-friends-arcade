...
| `worker/tests/room-code-reuse.test.mjs` | 방 코드 재사용 경계 — 이전 판·좌석 토큰이 넘어오지 않는지 |
...
|------|------|
| CHECK | 콜할 금액이 0일 때만 |
| BET | 1칩 이상. 보유 칩 이내이면서 아래 **유효 스택** 상한 이내 |
...
**유효 스택** — 헤즈업이라 상대가 받을 수 없는 금액은 걸어도 의미가 없다. 한 단계의 총 베팅액은
`min(내 베팅액 + 내 칩, 상대 베팅액 + 상대 칩)` 을 넘을 수 없고, 넘으면 `OVER_EFFECTIVE_STACK` 으로 거부된다.

- 상대가 **이미 올인**이면 상한 = 현재 최고 베팅액이라 레이즈·올인이 아예 열리지 않는다 — 콜·폴드뿐이다.
  (이 상한이 없으면 칩 0인 상대에게 "콜 0" 차례가 돌아가는 유령 라운드가 생긴다.)
- 내가 빅스택이면 올인 버튼 대신 BET/RAISE 상한이 곧 실질 올인이다(상대 잔여 칩까지).

콜되지 않은 초과 베팅은 항상 베팅한 쪽에 반환된다(폴드로 끝난 라운드 등).
...
단, **이월은 다음 라운드가 있어야 성립한다.** 동점인데 한쪽 칩이 0이면(올인 뒤 동점) 그 라운드는
오지 않으므로 이월 팟이 아무에게도 가지 않고 사라진다. 그래서 이 경우에만 그 자리에서 **반씩 나눈다**
(`lastResult.split`). 홀수 1칩은 칩이 적은 쪽에게 준다(같으면 후플레이어) — 나누기 때문에 오히려
탈락하는 일이 없게 하기 위해서다. 나눈 뒤 양쪽 모두 칩이 남으면 게임은 그대로 계속된다.
...
- **영속.** 하이버네이션 DO 는 ws 가 살아 있어도 in-memory 가 사라지므로 매 변화마다 storage 에 쓴다.
  모듈 상태는 코어가 `mod:<gameId>:` 네임스페이스로 감싸 저장한다.
- **방 코드 재사용 경계.** 4자리 코드는 재사용된다(`POST /api/rooms` → DO `/init`). `/init` 은
  라이브 연결이 없는 방을 새 방으로 넘기기 전에 그 방의 저장·in-memory 상태를 전부 지운다.
  안 지우면 새 손님이 이전 판을 물려받고, 이전 방의 좌석 토큰 보유자가 그 자리로 돌아온다.
