...
**P0 · P1 · P2 · P3 완료 — 다음은 P4(콘텐츠: 던전·일일 과제·도감)**
...
- **던전** — HP·방어 성장축이 처음으로 의미를 갖는 자리. 별도 규칙(생존 체크)로 설계
- 일일 과제 / 도감(만난 몬스터 기록 — 챕터 데이터에 이미 이름·아트가 있다)
- 챕터 6+ — `data/chapters.js` 에 데이터 추가만으로 확장

**게이트**: 하루에 두 번 이상 열어볼 이유가 생긴다.
...
- [ ] **캐릭터별 강화** — 지금은 강화가 계정 단위다. 캐릭터마다 레벨을 두면 편성에
      "누구를 키울까"라는 축이 하나 더 생긴다(P4 와 함께 검토).
- [ ] **유물 아이콘** — 지금은 이모지(🔨🧺🧭). imagegen 으로 교체하면 더 예뻐진다(P5).
- [ ] **사운드** — 아직 없다. P5.
