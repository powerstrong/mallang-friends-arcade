...
> - **사람 검증 남음**: 실기기 시각 확인(버블 위치·골든 발판 체감·힌트 위치·배경 전환 멀미·잠금 칩),
>   30초 실주행 감각, 멀티 2클라(메달·고스트 비표시·순위), 백그라운드 전환 복귀, 해금 임계값 체감.
> - R13(시크릿 해금)도 구현 완료(아래 상세). **남은 과제: R12뿐(보류 확정)** — 사유는 R12 절 참조.
> - R8 시뮬 근거: 평균(탭400ms) 61.9층·피버 1.27회·보호막 충전 1.0회 — 목표(판당 피버~1회) 이미 충족.
>   초보(600ms)는 피버 0회·게이지사망 1.5회로 빡빡하지만 무한의계단류 본연의 긴장이므로 유지.
>
> **R13 구현 상세 (2026-08-17 두 번째 로컬 세션):** 시크릿 직접선택 해금 —
> 라떼 강아지(콤보형) = **한 판 최대 콤보 50**, 민트 고양이(피버형) = **한 판 피버 2번**.
> 임계값은 dev/sim.js 분포로 결정: 콤보50 = 평균 65%/판·초보 0%(첫 시크릿, 실력 사다리),
> 피버2 = 평균 25%·상급 85%(두 번째, 도전 목표). 콤보 30~40은 평균에게 즉시(80~95%)라 탈락, 피버 3은 평균 0%라 탈락.
> 셋업 칩: 공개 3 + 잠금 실루엣 칩(???+🔒, 탭하면 조건 프리뷰 2.6s 후 복귀) + 랜덤.
> **랜덤 추첨 풀은 해금과 무관하게 5종 유지**(잠금 캐릭터를 랜덤으로 만나는 서프라이즈 보존, 멀티 방 배정 포함).
> 달성 시 결과 카드에 "🎉 새 친구 해금" 배너 + 칩 즉시 해제. 저장 `mallang-stairs:unlocks`.
> 검증: 헤드리스로 잠금 칩 6종 구성→프리뷰/복귀→55콤보 주행→해금 배너·저장·칩 재구성·선택 보존까지 통과.
...
### R12. 멀티 시작 동기화 정밀화 — **보류 확정 (2026-08-17)**

start 브로드캐스트 수신 시점 차이만큼(대개 <200ms) 클라 간 시작이 어긋난다. 서버 시각 기반 `startAt`을 주려면 [worker/src/room.js](../../worker/src/room.js) relay에 서버 타임스탬프 부가가 필요해 **모든 릴레이 게임이 공유하는 워커**를 건드린다. 멀티 2클라 사람 검증이 불가한 시기에 공유 인프라를 수정하는 리스크 대비 체감 이득(<200ms)이 작아 보류한다. 재개 조건: 멀티 사람 검증 체계가 갖춰지고, 실플레이에서 시작 어긋남이 실제로 체감된다는 관찰이 있을 때.

### R13. 시크릿 캐릭터 해금 조건 — **구현됨 (2026-08-17)**

상단 진행 현황의 "R13 구현 상세" 참조. 조건 변경 시 [characters.js](characters.js)의 `unlock` 필드(value·label)만 고치면 되고, 판정·UI는 game.js가 메타데이터를 읽는다. 임계값 재검토는 `node games/mallang-stairs/dev/sim.js`의 해금 분포 표를 근거로 할 것.
...
- 2026-08-17(같은 날): 로컬 세션에서 R1~R11 구현·검증 완료(상단 진행 현황 참조).
- 2026-08-17(같은 날, 후속): R13 시크릿 해금 구현(콤보 50/피버 2, 시뮬 근거) · R12 보류 확정. 백로그의 코드 과제 소진 — 남은 것은 사람 검증뿐.
